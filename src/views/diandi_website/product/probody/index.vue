<template>
  <div class="app-container">
    ...
    >
      <template slot="image_a" slot-scope="{ row }">
        <el-image
          style="width:80px; height: 80px"
          :src="row.image_a"
        />
      </template>
      <template slot="image_b" slot-scope="{ row }">
        <el-image
          style="width:80px; height: 80px"
          :src="row.image_b"
        />
...
export default {
  ...
  data() {
    return {
      ...
      tableColumns: [
        { label: '标题', prop: 'title' },
        { label: 'a图', prop: 'image_a', slot: 'image_a' },
        { label: 'b图', prop: 'image_b', slot: 'image_b' },
  ...
  methods: {
    ...
    getList() {
      ...
      proBodyList(that.filterInfo.data).then((response) => {
    ...
    handleCreate() {
      this.$router.push({
        name: 'product-probody-create'
      })
    },
    // 删除
    delectRow(row) {
      const that = this
      deleteProBody(row.id).then((response) => {
