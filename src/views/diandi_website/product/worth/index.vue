...
import {
  sysWorthList,
  deletesysWorth
} from '@/views/diandi_website/api/worth.js'
export default {
  components: {},
  data() {
    return {
  ...
  methods: {
    ...
    getList() {
      ...
      sysWorthList(that.filterInfo.data).then((response) => {
    ...
    handleCreate() {
      this.$router.push({
        name: 'product-worth-create'
      })
    },
    // 删除
    delectRow(row) {
      const that = this
      deletesysWorth(row.id).then((response) => {
