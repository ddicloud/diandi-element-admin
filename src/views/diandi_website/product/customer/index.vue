...
export default {
  ...
  data() {
    return {
      ...
      tableColumns: [
        ...
        { label: '图片', prop: 'image', slot: 'image' },
      ...
      filterInfo: {
        ...
        fieldList: [
          { label: '标题', type: 'input', value: 'WebsiteProCustomer[title]' }
        ]
      },
      total: 0,
      list: [],
  ...
  methods: {
    ...
    getList() {
      ...
      proCustomerList(that.filterInfo.data).then((response) => {
    ...
    handleCreate() {
      this.$router.push({
        name: 'product-customer-create'
      })
    },
    // 删除
    delectRow(row) {
      const that = this
      deleteProCustomer(row.id).then((response) => {
