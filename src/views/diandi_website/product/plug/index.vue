...
export default {
  ...
  data() {
    return {
      ...
      filterInfo: {
        ...
        fieldList: [
          { label: '标题', type: 'input', value: 'WebsiteProPlug[title]' }
        ]
      },
      total: 0,
      list: [],
  ...
  methods: {
    ...
    getList() {
      ...
      proPlugList(that.filterInfo.data).then((response) => {
    ...
    handleCreate() {
      this.$router.push({
        name: 'product-plug-create'
      })
    },
    // 删除
    delectRow(row) {
      const that = this
      deleteProPlug(row.id).then((response) => {
