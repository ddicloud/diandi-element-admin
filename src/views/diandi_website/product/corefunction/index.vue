...
export default {
  ...
  data() {
    return {
      ...
      tableColumns: [
        ...
        { label: 'logo', prop: 'logo', slot: 'logo' },
      ...
      filterInfo: {
        ...
        fieldList: [
          { label: '标题', type: 'input', value: 'WebsiteProCore[title]' }
        ]
      },
      total: 0,
      list: [],
  ...
  methods: {
    ...
    getList() {
      ...
      proCoreList(that.filterInfo.data).then((response) => {
    ...
    amend(row) {
      this.$router.push({
        name: 'product-corefunction-update',
        params: {
          id: row.id,
          rowData: row
        }
    ...
    handleCreate() {
      this.$router.push({
        name: 'product-corefunction-create'
      })
    },
    // 删除
    delectRow(row) {
      const that = this
      deleteProCore(row.id).then((response) => {
