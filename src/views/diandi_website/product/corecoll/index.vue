...
export default {
  ...
  data() {
    return {
      ...
      filterInfo: {
        ...
        fieldList: [
          { label: '标题', type: 'input', value: 'WebsiteProSelling[title]' }
        ]
      },
      total: 0,
      list: [],
  ...
  methods: {
    ...
    getList() {
      ...
      proSellingList(that.filterInfo.data).then((response) => {
    ...
    handleCreate() {
      this.$router.push({
        name: 'product-corecoll-create'
      })
    },
    // 删除
    delectRow(row) {
      const that = this
      deleteProSelling(row.id).then((response) => {
