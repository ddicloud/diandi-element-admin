...
export default {
  ...
  data() {
    return {
      ...
      tableColumns: [
        ...
        { label: '是否推荐', prop: 'is_recommend', slot: 'is_recommend' },
        { label: '产品价格', prop: 'price' },
  ...
  methods: {
    ...
    getList() {
      ...
      productPriceList(that.filterInfo.data).then((response) => {
    ...
    delectRow(row) {
      const that = this
      deleteproductPrice(row.id).then((response) => {
